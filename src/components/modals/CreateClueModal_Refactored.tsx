...
export default function CreateClueModal_Refactored({ isOpen, onClose, investigationId, onSaved }: Props) {
  ...
  return (
    <div className="createclue-modal-overlay">
      <div className="createclue-modal">
        <div className="createclue-header">
          <div className="flex gap-2">
            <button className={`createclue-tab-btn ${tab === 0 ? 'active' : ''}`} onClick={() => setTab(0)}>Básico</button>
            <button className={`createclue-tab-btn ${tab === 1 ? 'active' : ''}`} onClick={() => setTab(1)}>Mídia</button>
            <button className={`createclue-tab-btn ${tab === 2 ? 'active' : ''}`} onClick={() => setTab(2)}>Validação</button>
            <button className={`createclue-tab-btn ${tab === 3 ? 'active' : ''}`} onClick={() => setTab(3)}>Preview</button>
            <button className={`createclue-tab-btn ${tab === 4 ? 'active' : ''}`} onClick={() => setTab(4)}>Config</button>
            <button className={`createclue-tab-btn ${tab === 5 ? 'active' : ''}`} onClick={() => setTab(5)}>Publicar</button>
          </div>
          <div>
            <button onClick={onClose} className="createclue-close">Fechar</button>
          </div>
        </div>
